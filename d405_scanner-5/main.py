...
def step_process(
    ...
    # Boden entfernen (RANSAC)
    log.info("Entferne Bodenebene (RANSAC)...")
    final_pcd = processor.remove_ground_plane(final_pcd)
...
def step_analysis(
    ...
    viz_path = cfg.paths.reports_dir / f"scan_3d_{ts}.html"
    analyzer.save_3d_visualization(stats, viz_path)
    log.info(f"3D-Visualisierung: file://{viz_path.resolve()}")
